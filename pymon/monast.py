...
class Monast:
    ...
    def __init__(self, configFile):
        ...
        self.eventHandlers = {
            ...
            'ContactStatusDetail' : self.handlerEventContactStatusDetail,
    ...
    def _requestAsteriskConfig(self, servername):
        ...
        ## Peers (PJSIP) :: Process results via handlerEventEndpointList
    ...
    def handlerEventEndpointList(self, ami, event):
        ...
        if user:
            ## Peer (PJSIP) :: Process result via handlerEventEndpointDetail, handlerEventContactStatusDetail
            log.debug("Server %s :: Requesting PJSIP EndpointDetail..." % ami.servername)
            server.pushTask(server.ami.sendDeferred, {'action': 'pjsipshowendpoint', 'endpoint': objectname}) \
                .addCallback(server.ami.errorUnlessResponse) \
                .addErrback(self._onAmiCommandFailure, servername, "Error Requesting PJSIP EndpointDetail")
    ...
    def handlerEventEndpointDetail(self, ami, event):
        ...
        try:
            ...
            if callerid == ' <>':
                callerid = server.status.peers['PJSIP'][objectname].callerid
        except:
            callerid = server.status.peers['PJSIP'][objectname].callerid
    ...
    def handlerEventContactStatusDetail(self, ami, event):
        log.debug("Server %s :: Processing Event ContactStatusDetail..." % ami.servername)
        server     = self.servers.get(ami.servername)
        objectname = event.get('endpointname')
        time       = event.get('roundtripusec')
        status     = event.get('status')

        peer_time = server.status.peers['PJSIP'][objectname].time
        if time > peer_time and peer_time != -1:
            time = peer_time
